...
if ($_FILES && !empty($_FILES['file']['name'])){
    ...
    if (in_array($fileUpload['type'], $allowedTypes)){
        if (move_uploaded_file($fileUpload['tmp_name'],
            __DIR__."/uploads/{$newFilename}")){
            echo "<p class='trigger accept'>Arquivo enviado com sucesso!</p>";
        }else{
            echo "<p class='trigger error'>Erro ao enviar o arquivo!</p>";
        }
    }else {
        echo "<p class='trigger error'>Tipo de arquivo não permitido 
            (Enviar jpg, jpeg, png e pdf)</p>";

    }

}
elseif ($getPost){
    echo "<p class='trigger warning'>Arquivo é muito grande!</p>";
}
else{
    if ($_FILES){
